import { Heart, Star, Calendar, Users, Check, ChevronDown, ChevronUp, MapPin, Clock, Shield, Wifi, UtensilsCrossed, Sparkles, Copy, Phone, ArrowLeft } from 'lucide-react';
import { useEffect, useState, useContext, useRef } from 'react';
import { toast } from 'react-toastify';
import { PackageCardsSkeleton } from '../components/LoadingSkeleton';
...
const Packages = () => {
    ...
    const handleConfirmBooking = async () => {
        ...
        try {
            const checkoutDate = getCheckoutDate(checkInDate, bookingPackage.duration);
            ...
            const { data } = await API.post('/bookings', {
                ...
                totalPrice: Math.round(bookingPackage.price * (1 - (bookingPackage.discount || 0) / 100)) * roomsBooked,
    ...
    const fetchPackages = async () => {
        try {
            setLoading(true);
            const data = await cachedGet('/packages');
            setPackages(data);
        } catch (error) {
            ...
            <div className="min-h-screen bg-white pt-4">
                <div className="container mx-auto px-6 py-12">
                    <div className="mb-8 space-y-3 text-center">
                        ...
                        <div className="mx-auto h-10 w-72 animate-pulse rounded-2xl bg-slate-200" />
                        <div className="mx-auto h-4 w-96 animate-pulse rounded-full bg-slate-200" />
                    </div>
                    <PackageCardsSkeleton count={3} />
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="min-h-screen bg-white pt-6">
            ...
            <section className="py-12 bg-white">
                <div className="container mx-auto px-6">
                    <div className="text-center mb-12">
                        <span className="text-black uppercase tracking-[0.3em] text-xs sm:text-sm md:text-base font-bold mb-4 block">
                            Our Offerings
                        </span>
                        <h1 className="text-black text-xl md:text-2xl font-serif tracking-tight mb-2">
                            Curated Packages
                        </h1>
                        {/* <p className="text-slate-600 text-lg max-w-2xl mx-auto">
                            Choose from our selection of luxurious resort packages designed to create unforgettable experiences.
                        </p> */}
                    </div>
                    ...
                    {packages.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            {packages.map((pkg, idx) => (
                                <div
                                    key={pkg._id}
                                    className="relative flex flex-col bg-white rounded border border-slate-100/80 shadow-[0_8px_30px_rgb(0,0,0,0.015)] hover:shadow-[0_20px_45px_rgba(19,73,65,0.09)] hover:border-emerald-800/10 transition-all duration-500 overflow-hidden group cursor-pointer"
                                ...
                                >
                                    ...
                                    <div className="relative h-64 overflow-hidden bg-slate-100">
                                        ...
                                        />
                                        <div className="absolute inset-0 bg-gradient-to-t from-slate-900/40 via-transparent to-transparent opacity-50 pointer-events-none" />
                                        
                                        {/* Floating Badge (Best Seller vs Recommended) */}
                                        <span className="absolute top-4 left-4 bg-[#134941] text-white text-[9px] font-bold uppercase tracking-wider px-2.5 py-1.5 rounded-md shadow-sm">
                                            {idx === 0 ? 'Best Seller' : 'Recommended'}
                                        </span>

                                        {pkg.discount > 0 && (
                                            <div className="absolute top-4 left-24 bg-gradient-to-r from-red-600 to-orange-500 text-white font-bold text-[9px] uppercase tracking-wider px-2.5 py-1.5 rounded-md shadow-md">
                                                -{pkg.discount}%
                                            </div>
                                        )}
                                        ...
                                        <button
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                toast.info(`Added ${pkg.name} to wishlist!`);
                                            }}
                                            className="absolute top-4 right-4 bg-white/80 hover:bg-white text-slate-700 hover:text-red-500 p-2.5 rounded-full shadow-md backdrop-blur-sm transition-all duration-300 hover:scale-110 active:scale-95"
                                        >
                                    ...
                                    <div className="p-5 flex-1 flex flex-col">
                                        ...
                                        <div className="flex justify-between items-start gap-3 mb-2">
                                            <h3 className="text-base font-serif text-slate-900 group-hover:text-[#134941] transition-colors duration-300 font-bold leading-snug line-clamp-2">
                                                {pkg.name}
                                            </h3>
                                            <span className="flex-shrink-0 bg-emerald-50/60 border border-emerald-100 text-[#134941] text-xs font-semibold px-2 py-0.5 rounded-md">
                                                {parseDurationAbbreviation(pkg.duration)}
                                            </span>
                                        </div>

                                        {/* Capacity & Highlights Subtitle */}
                                        <div className="flex items-center gap-2 text-[10px] text-slate-400 font-light mb-4">
                                            <span>👥 Up to {pkg.capacity} Guests</span>
                                            <span>•</span>
                                            <span className="text-emerald-700 font-medium">Free Cancellation</span>
                                        </div>

                                        <p className="text-slate-500 text-xs leading-relaxed mb-5 font-light line-clamp-2">
                                            {pkg.description}
                                        </p>

                                        {/* Structured Inclusions Dual-Column Grid */}
                                        <div className="grid grid-cols-2 gap-4 mb-5 pb-4 border-b border-slate-100">
                                            {/* Column 1: Standard Amenities */}
                                            <div>
                                                <p className="text-[8px] uppercase tracking-wider text-slate-400 font-bold mb-2">Included Amenities</p>
                                                <ul className="space-y-1.5">
                                                    {(pkg.amenities && pkg.amenities.length > 0 ? pkg.amenities.slice(0, 3) : ['Luxury Suite Stay', 'Free WiFi Access', 'Pool & Gym Access']).map((amenity, i) => (
                                                        <li key={i} className="flex items-center gap-1.5 text-xs text-slate-500 font-light">
                                            ...
                                            {/* Column 2: Specific Experience Highlights */}
                                            <div>
                                                <p className="text-[8px] uppercase tracking-wider text-slate-400 font-bold mb-2">Package Highlights</p>
                                                <ul className="space-y-1.5">
                                                    {(pkg.highlights && pkg.highlights.length > 0 ? pkg.highlights.slice(0, 3) : ['Bespoke Dining', 'Resort Spa Credit', 'Guided Local Tour']).map((hl, i) => (
                                                        <li key={i} className="flex items-center gap-1 text-xs text-slate-600 font-light">
                                                            <span className="text-emerald-600 font-medium text-[9px] flex-shrink-0">✓</span>
                                        ...
                                        {/* Pricing & CTA Option Box Footer */}
                                        <div className="flex items-stretch justify-between gap-3 mt-auto mb-2">
                                            {/* Flex Booking Option Box */}
                                            <div className="bg-slate-50 border border-slate-100 p-2.5 rounded-xl flex-1 max-w-[50%] flex flex-col justify-center">
                                                <p className="text-[8px] uppercase tracking-wider text-slate-400 font-bold">Booking Flex</p>
                                                <p className="text-[10px] font-bold text-slate-800 leading-tight">Pay at Property</p>
                                                <p className="text-[8px] text-emerald-700 font-medium">No Advance Required</p>
                                            </div>

                                            {/* Price and CTA Button */}
                                            <div className="flex flex-col justify-between items-end text-right">
                                                <div>
                                                    <span className="text-[8px] uppercase tracking-wider text-slate-400 font-bold block leading-none mb-0.5">Starting From</span>
                                                    {pkg.discount > 0 ? (
                                                        <div className="flex items-baseline gap-1.5 justify-end">
                                                            <span className="text-lg font-serif text-[#134941] font-bold">₹{Math.round(pkg.price * (1 - pkg.discount / 100)).toLocaleString()}</span>
                                                            <span className="text-xs text-slate-400 line-through font-light">₹{pkg.price.toLocaleString()}</span>
                                                        </div>
                                                    ) : (
                                                        <span className="text-lg font-serif text-slate-900 font-bold">₹{pkg.price.toLocaleString()}</span>
                                                    )}
                                                </div>
                                                <button 
                                                    onClick={(e) => {
                                                    ...
                                                    className="bg-[#134941] text-white hover:bg-[#1b6b5f] text-[10px] font-bold uppercase tracking-wider px-4 py-2 rounded-lg shadow-sm hover:shadow-md transition-all duration-300 mt-1"
                                                >
                                                    Book Now
                                                </button>
                                            </div>
                                    ...
                                    {/* Promotional Accent Ribbon */}
                                    <div className="bg-emerald-50/50 border-t border-emerald-100 px-5 py-2 flex items-center text-[10px] text-[#134941]">
                                        <span className="mr-1.5">🏷️</span>
                                        <span>Use Coupon <strong className="font-extrabold uppercase">WELCOME</strong> for extra savings</span>
                                    </div>
                                </div>
                            ))}
                        </div>
            ...
            {/* Package Detail Section (Inline) — Premium Redesign */}
            {selectedPackage && (() => {
                ...
                const daysCount = getDaysCount(selectedPackage.duration);
                const finalPrice = Math.round(selectedPackage.price * (1 - (selectedPackage.discount || 0) / 100));
                const savedAmount = Math.round(selectedPackage.price * (selectedPackage.discount || 0) / 100);
                ...
                return (
                    <section ref={detailSectionRef} className="bg-white border-t border-slate-100">
                        ...
                        <div className="relative h-[340px] sm:h-[420px] lg:h-[480px] overflow-hidden">
                            ...
                            {/* Wishlist Button */}
                            <button
                                onClick={() => toast.info(`Added ${selectedPackage.name} to wishlist!`)}
                                ...
                                <div className="max-w-7xl mx-auto">
                                    <div className="flex flex-wrap items-center gap-2 mb-3">
                                        <span className="bg-white/20 backdrop-blur-sm text-white text-[10px] font-bold uppercase tracking-wider px-3 py-1 rounded-full border border-white/20">
                                            {parseDurationAbbreviation(selectedPackage.duration)} Package
                                        </span>
                                    ...
                                    <h2 className="text-2xl sm:text-3xl lg:text-4xl font-serif text-white leading-tight mb-2 drop-shadow-lg">{selectedPackage.name}</h2>
                                    <p className="text-white/70 text-sm sm:text-base font-light max-w-2xl leading-relaxed line-clamp-2">{selectedPackage.description}</p>
                                </div>
                            </div>
                        </div>
                            ...
                            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10">
                                <div className="flex items-center gap-3 py-4 overflow-x-auto scrollbar-hide -mx-1 px-1">
                                    <div className="flex items-center gap-2 bg-[#134941]/5 border border-[#134941]/10 text-[#134941] px-3.5 py-2 rounded-full flex-shrink-0">
                                        <Calendar size={14} />
                                        <span className="text-xs font-semibold">{selectedPackage.duration}</span>
                                    </div>
                                    <div className="flex items-center gap-2 bg-slate-50 border border-slate-200/60 text-slate-600 px-3.5 py-2 rounded-full flex-shrink-0">
                                        <Users size={14} />
                                        <span className="text-xs font-semibold">Up to {selectedPackage.capacity} Guests</span>
                                    </div>
                                ...
                                <div className="w-full lg:w-[63%]">
                                    ...
                                    {activeDetailTab === 'itinerary' && (
                                        <div className="space-y-3">
                                            ...
                                            {generateItinerary(selectedPackage, daysCount).map((item) => (
                                        ...
                                        <div className="space-y-8">
                                            ...
                                            <div>
                                                ...
                                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                    {(selectedPackage.amenities && selectedPackage.amenities.length > 0 ? selectedPackage.amenities : ['Luxury Suite Stay', 'Daily Gourmet Breakfast', 'High-Speed WiFi', 'Pool & Fitness Access']).map((amenity, i) => (
                                            ...
                                            <div>
                                                ...
                                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                    {(selectedPackage.highlights && selectedPackage.highlights.length > 0 ? selectedPackage.highlights : ['Bespoke Private Dinner', 'Complimentary Spa Voucher', 'Guided Nature Trek']).map((highlight, i) => (
                                                ...
                                                <div className="flex items-end justify-between">
                                                    <div>
                                                        ...
                                                        <div className="flex items-baseline gap-2">
                                                            ...
                                                            {selectedPackage.discount > 0 && (
                                                                <span className="text-sm text-white/50 line-through">₹{selectedPackage.price.toLocaleString()}</span>
                                                            )}
                                                        </div>
                                                    ...
                                                    {selectedPackage.discount > 0 && (
                                                        <div className="bg-white/20 backdrop-blur-sm px-3 py-1.5 rounded-lg text-center">
                                                            <p className="text-lg font-bold">{selectedPackage.discount}%</p>
                                            ...
                                            <div className="p-5 space-y-4">
                                                ...
                                                <div className="space-y-2 pt-3 border-t border-slate-100">
                                                    <div className="flex justify-between text-xs">
                                                        ...
                                                        <span className="text-slate-700 font-medium">₹{selectedPackage.price.toLocaleString()}</span>
                                                    </div>
                                                    {selectedPackage.discount > 0 && (
                                                        <div className="flex justify-between text-xs">
                                                            <span className="text-emerald-600">Discount ({selectedPackage.discount}%)</span>
                                                ...
                                                {/* Book Now CTA */}
                                                <button 
                                                    onClick={() => handleBookNowClick(selectedPackage)}
                        ...
                        <div className="max-w-4xl mx-auto">
                            ...
                            <div className="flex items-center justify-between mb-8">
                                <div>
                                    ...
                                    <h2 className="text-3xl md:text-4xl font-serif text-black">Book {bookingPackage.name}</h2>
                                </div>
                                <button
                                    onClick={() => {
                                        setBookingPackage(null);
                                                    ...
                                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                                        ...
                                                        <div>
                                                            ...
                                                            <div className="w-full rounded-lg border border-slate-100 bg-slate-50 px-4 py-3 text-sm text-slate-600 font-medium select-none">
                                                                {checkInDate ? getCheckoutDate(checkInDate, bookingPackage.duration).toLocaleDateString() : 'Select check-in first'}
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                                        ...
                                                        <div>
                                                            ...
                                                            <div className="flex items-center gap-3 mt-1">
                                                                ...
                                                                <button
                                                                    type="button"
                                                                    onClick={() => setAdults(prev => Math.min(bookingPackage.capacity, prev + 1))}
                                                                    className="w-10 h-10 rounded-lg border flex items-center justify-center text-slate-600 hover:bg-slate-50 transition font-bold"
                                                                >
                                                                    +
                                                                </button>
                                ...
                                <div className="lg:col-span-1">
                                    <div className="bg-white rounded-2xl shadow-lg border border-slate-100 overflow-hidden sticky top-28">
                                        ...
                                        <div className="p-6 space-y-4">
                                            <h3 className="text-xl font-serif text-slate-900">{bookingPackage.name}</h3>
                                            <div className="space-y-2 text-sm">
                                                <div className="flex items-center justify-between text-slate-600">
                                                    <span>📅 Duration</span>
                                                    <span className="font-medium text-slate-900">{bookingPackage.duration}</span>
                                                </div>
                                                <div className="flex items-center justify-between text-slate-600">
                                                    <span>👥 Capacity</span>
                                                    <span className="font-medium text-slate-900">Up to {bookingPackage.capacity} guests</span>
                                                </div>
                                                {checkInDate && (
                                                    <div className="flex items-center justify-between text-slate-600">
                                                ...
                                                {checkInDate && (
                                                    <div className="flex items-center justify-between text-slate-600">
                                                        ...
                                                        <span className="font-medium text-slate-900">{getCheckoutDate(checkInDate, bookingPackage.duration).toLocaleDateString()}</span>
                                                    </div>
                                                )}
                                            </div>
                                            ...
                                            <div className="pt-4 border-t space-y-2">
                                                <div className="flex justify-between text-sm text-slate-600">
                                                    ...
                                                    <span className="font-medium text-slate-900">₹{bookingPackage.price.toLocaleString()}</span>
                                                </div>
                                                {bookingPackage.discount > 0 && (
                                                    <div className="flex justify-between text-sm text-green-600">
                                                        <span>Discount ({bookingPackage.discount}%)</span>
                                                        <span className="font-medium">-₹{Math.round(bookingPackage.price * bookingPackage.discount / 100).toLocaleString()}</span>
                                                    </div>
                                                )}
                                                {roomsBooked > 1 && (
                                                    <div className="flex justify-between text-sm text-slate-600">
                                                        <span>× {roomsBooked} rooms</span>
                                                        <span className="font-medium text-slate-900">₹{(Math.round(bookingPackage.price * (1 - (bookingPackage.discount || 0) / 100)) * roomsBooked).toLocaleString()}</span>
                                                    </div>
                                                )}
                                                <div className="flex justify-between items-center font-bold text-slate-900 pt-3 border-t text-base">
                                                    <span>Total</span>
                                                    <span className="text-[#1b6b5f] text-xl">₹{(Math.round(bookingPackage.price * (1 - (bookingPackage.discount || 0) / 100)) * roomsBooked).toLocaleString()}</span>
                                                </div>
                                            </div>

                                            {bookingPackage.discount > 0 && (
                                                <div className="bg-green-50 border border-green-200 rounded-lg px-3 py-2 text-center">
                                                    <span className="text-green-700 text-xs font-bold">🎉 You save ₹{(Math.round(bookingPackage.price * bookingPackage.discount / 100) * roomsBooked).toLocaleString()}!</span>
                                                </div>
                                            )}
                                        </div>
                                    </div>
